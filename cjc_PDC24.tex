...
As an example, we investigated the monolithic solver approach applied
...
\begin{align*}
  ...
  k=v,c,r
\end{align*}
where $S_{q_v}$, $S_{q_c}$ and $S_{q_r}$ are reaction terms between
the species governing condensation/evaporation/raindrop formation and
their associated latent heat sinks/sources $S_\theta$ (see
\cite{zerroukat2015moist} for more details). We discretised all the
tracers in the same DG1 ``pressure'' finite element space.
